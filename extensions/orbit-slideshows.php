<?php
/**
 * Orbit Slideshows - A script to set up slideshows for use with Orbit.
 *
...
function slideshow_meta_box( $metabox ) {
    $metabox = array();
    $metabox['slide_caption'] = array( 'name' => 'slide_caption', 'title' => sprintf( __( 'Slide Caption', hybrid_get_textdomain() ) ), 'type' => 'text', 'description' => '<p class="howto">To deactivate a slide, set its status to "Pending Review" or "Draft"</p>' );
...
function slideshow_enqueue_script() {
    wp_enqueue_script('orbit', trailingslashit( HYBRID_ALT_ADMIN_JS ) . 'jquery.orbit.min.js', array('jquery'));
}
            ...
            if ( has_post_thumbnail() ) {
        	      if ( $meta_caption ) {
        	          $attr = array(
            	          'alt' => "HTML Captions",
                    		'title'	=> get_the_title(),
                    		'data-caption' => '#htmlCaption'.$iteration
                    );
                } else {
            	      $attr = array(
                    		'title'	=> get_the_title(),
